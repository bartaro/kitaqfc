...
partial class Parser
{
    ...
    int? CurrentFunctionBankForStringLiterals = null;
    ...
    // Use the active function's selected bank when present, otherwise the current file-scope bank pragma.
    int GetCurrentStringLiteralBank()
    {
        return CurrentFunctionBankForStringLiterals ?? CurrentPragmaBank;
    }

    // Deduplicate string text only within the selected bank; null text is equivalent to empty.
    string GetStringLiteralPoolKey(string value)
    {
        return GetCurrentStringLiteralBank().ToString() + "\n" + (value ?? "");
    }
    ...
    // Round upward with a bit-mask formula; callers must supply a positive power-of-two alignment for aligned results.
    ...
    // Use recorded aggregate alignment or infer one/two-byte alignment from storage size for other types.
    int GetAggregateAlignment(CType type)
    {
        if (type == null) return 1;
        if (type.IsStructOrUnion && TryGetAggregateDecl(type, out AggregateDeclInfo info))
            return Math.Max(1, info.Alignment);
        return GetTypeStorageSize(type) >= 2 ? 2 : 1;
    }

    // Estimate parser-side storage using byte enums, two-byte pointers and known aggregate layouts.
    // Unknown or nonliteral expression dimensions yield zero; negative array dimensions are clamped to zero.
    int GetTypeStorageSize(CType type)
    {
        ...
        {
            int elem = GetTypeStorageSize(type.Subtype);
            return elem <= 0 ? 0 : elem * Math.Max(0, type.Dimension);
        }
        if (type.Tag == CTypeTag.ArrayWithDimensionExpression)
        {
            if (type.DimensionExpression != null && type.DimensionExpression.Match(Tag.Integer, out int dim))
            {
                int elem = GetTypeStorageSize(type.Subtype);
        ...
        if (type.IsStructOrUnion && TryGetAggregateDecl(type, out AggregateDeclInfo info))
            return Math.Max(0, info.TotalSize);
        return 0;
    }

    // Assign aligned struct offsets or zero union offsets, then compute aggregate extent and alignment.
    // Packed layouts suppress field/tail padding; forced alignment replaces the natural aggregate alignment.
    FieldInfo[] LayoutAggregateFields(bool isUnion, FieldInfo[] rawFields, bool isPacked, int forcedAlign, out int totalSize, out int alignment)
    {
        rawFields = rawFields ?? Array.Empty<FieldInfo>();
        FieldInfo[] laidOut = new FieldInfo[rawFields.Length];
        int offset = 0;
        int naturalAlign = 1;
        int maxSize = 0;
        ...
        for (int i = 0; i < rawFields.Length; i++)
        {
            ...
            int fieldSize = GetTypeStorageSize(field.Type);
            int fieldAlign = isPacked ? 1 : GetAggregateAlignment(field.Type);
            ...
            laidOut[i] = new FieldInfo(field.Type, field.Name, fieldOffset);
        }
        ...
        alignment = forcedAlign > 0 ? forcedAlign : naturalAlign;
        if (!isPacked)
    ...
    // Prefer a configured function-bank override, otherwise retain the current pragma bank for literal pooling.
    int ResolveFunctionStringLiteralBank(string functionName)
    {
        if (!string.IsNullOrEmpty(functionName) && Program.TryGetFunctionBankOverride(functionName, out int overrideBank))
            return overrideBank;
        return CurrentPragmaBank;
    }
...
Expr ParseDeclaration()
    {
        ...
        if (TryParseName("typedef"))
        {
            ...
            if (sawStructOrUnion)
            {
                ...
                if (TryParse(TokenType.LBRACE))
                {
                    ...
                    int aggSize;
                    int aggAlignment;
                    FieldInfo[] laidOutFields = LayoutAggregateFields(isUnion, parsedFields, aggPacked, aggAlign, out aggSize, out aggAlignment);
                    extraDecl = Make(tag, tagName, laidOutFields, aggPacked ? 1 : 0, aggAlign);
                    RegisterAggregateDecl(tagName, isUnion, laidOutFields, aggPacked, aggAlign);
        ...
        else
        {
            ...
            if (type.IsStructOrUnion && TryParse(TokenType.LBRACE))
            {
                ...
                int aggSize;
                int aggAlignment;
                FieldInfo[] laidOutFields = LayoutAggregateFields(type.Tag == CTypeTag.Union, parsedFields, type.IsPackedAggregate, type.ForcedAlign, out aggSize, out aggAlignment);
                RegisterAggregateDecl(type.Name, type.Tag == CTypeTag.Union, laidOutFields, type.IsPackedAggregate, type.ForcedAlign);
                return Make(tag, type.Name, laidOutFields, type.IsPackedAggregate ? 1 : 0, type.ForcedAlign);
            }
            else
            {
                string name = ExpectAnyName();
                ...
                if (TryParse(TokenType.LPAREN))
                {
                    ...
                    if (!TryParse(TokenType.RPAREN))
                    {
                        ...
                        {
                            CType fieldType = ExpectType();
                            if (fieldType.IsSimple && fieldType.SimpleType == CSimpleType.Void && PeekToken().Tag == TokenType.RPAREN && fields.Count == 0)
                            {
                                ConsumeToken();
                                break;
                            }
                    ...
                    // Save the surrounding string-bank context so a function-specific selection lasts only while parsing this body.
                    int? prevFunctionStringBank = CurrentFunctionBankForStringLiterals;
                    CurrentFunctionNameForStaticLocals = name;
                    CurrentFunctionStaticDecls = new List<Expr>();
                    CurrentFunctionBankForStringLiterals = ResolveFunctionStringLiteralBank(name);
                    ...
                    {
                        CurrentFunctionBankForStringLiterals = prevFunctionStringBank;
                    ...
                    if (TryParse(TokenType.EQUAL))
                    {
                        ...
                        if (region.Tag == MemoryRegionTag.ProgramRom || type.IsConst)
                        {
                            ...
                            if (type.IsArray)
                                ...
                                values.Add(ParseReadonlyInitializerValue(ref type));
    ...
    Expr ParseStatement(bool allowLong)
    {
        ...
        else if (TryParseName("__asm"))
        {
            ...
            while (!TryParse(TokenType.RBRACE))
            {
                ...
                else if ((string.Equals(symbol, "ASL", StringComparison.OrdinalIgnoreCase) ||
                          string.Equals(symbol, "LSR", StringComparison.OrdinalIgnoreCase) ||
                          string.Equals(symbol, "ROL", StringComparison.OrdinalIgnoreCase) ||
                          string.Equals(symbol, "ROR", StringComparison.OrdinalIgnoreCase)) &&
                         TryParseName("A"))
                {
                    // 6502 explicit accumulator spelling is equivalent to the
                    // operand-less form. Do not resolve A as a memory symbol.
                ...
                {
                    AddressMode operandMode = IsRelativeBranchMnemonic(symbol) ? AddressMode.Relative : AddressMode.Absolute;
                    AsmOperand operand = ParseAssemblyOperand(operandMode);
                    if (TryParse(TokenType.COMMA))
                    {
    ...
    // Newlines may occur after inline assembly even in a C initializer.
    void SkipInitializerNewlines() { while (TryParse(TokenType.NEWLINE)) { } }
    ...
    // Compute field offsets, aggregate size and alignment before caching the FC aggregate description.
    void RegisterAggregateDecl(string name, bool isUnion, FieldInfo[] fields, bool isPacked, int forcedAlign)
    {
        if (string.IsNullOrEmpty(name)) return;
        int totalSize;
        int alignment;
        fields = LayoutAggregateFields(isUnion, fields, isPacked, forcedAlign, out totalSize, out alignment);
    ...
    // Recognize the eight 6502 conditional branches after trimming and case normalization.
    static bool IsRelativeBranchMnemonic(string mnemonic)
    {
        switch ((mnemonic ?? "").Trim().ToUpperInvariant())
        {
            case "BPL":
            case "BMI":
            case "BVC":
    ...
    Expr ParsePrimaryExpr()
    {
        ...
        else if (TryParseString(out s))
        {
            ...
            if (!StringLiteralNameByValue.TryGetValue(poolKey, out name))
            {
                ...
                // Place this pooled literal in the active function/pragma bank; the pool key also includes that bank.
                StringLiteralDeclarations.Add(Make(Tag.Bank, GetCurrentStringLiteralBank(), Make(Tag.ReadonlyData, CType.MakeArray(CType.UInt8, s.Length + 1), name, values)));
            }
    ...
    bool TryParseType(out CType type)
    {
        ...
        // Collect signedness prefixes before the scalar spelling. Signed selects s8/s16 below;
        // void, enum, struct and union reject either prefix, and typedef lookup retains the alias type.
        bool sawUnsigned = false;
        bool sawSigned = false;
        while (true)
        {
            if (TryParseName("unsigned")) { sawUnsigned = true; continue; }
            if (TryParseName("signed")) { sawSigned = true; continue; }
            break;
        }
        ...
        if (TryParseName("void"))
        {
            ...
            {
                ParserError(ErrorCode.ExpectedType, "'void' cannot be combined with signed/unsigned");
            }
            type = CType.Void;
        }
        else if (TryParseName("uint8_t") || TryParseName("u8") || TryParseName("char") || TryParseName("bool"))
        {
            type = sawSigned ? CType.Int8 : CType.UInt8;
        }
        else if (TryParseName("uint16_t") || TryParseName("u16") || TryParseName("short"))
        {
            type = sawSigned ? CType.Int16 : CType.UInt16;
        }
        else if (TryParseName("enum"))
        {
            if (sawUnsigned || sawSigned)
            {
                ParserError(ErrorCode.ExpectedType, "'enum' cannot be combined with signed/unsigned");
            }
        ...
        else if (TryParseName("struct"))
        {
            if (sawUnsigned || sawSigned)
            {
                ParserError(ErrorCode.ExpectedType, "'struct' cannot be combined with signed/unsigned");
            }
            string name = ExpectAnyName();
        ...
        else if (TryParseName("union"))
        {
            if (sawUnsigned || sawSigned)
            {
                ParserError(ErrorCode.ExpectedType, "'union' cannot be combined with signed/unsigned");
            }
            string name = ExpectAnyName();
