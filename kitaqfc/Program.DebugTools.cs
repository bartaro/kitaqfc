...
static partial class Program
{
    ...
    static bool TryRunKqHelpCommand(string[] argsArray)
    {
        ...
        if (argsArray.Length < 2)
        {
            ...
            Console.Error.WriteLine("usage: kitaqfc kqhelp <KQxxxx>");
            Environment.Exit(1);
            return true;
        }
    ...
    static bool TryRunSymbolFindCommand(string[] argsArray)
    {
        ...
        if (argsArray.Length < 2)
        {
            ...
            Console.Error.WriteLine("usage: kitaqfc symfind <pattern> [--map=<.map>] [--dbg=<.dbg>] [--regex] [--ignore-case] [--out=<file>|-]");
            Environment.Exit(1);
            return true;
        }
    ...
    static bool TryRunSourceToAsmCommand(string[] argsArray)
    {
        ...
        if (argsArray.Length < 2)
        {
            ...
            Console.Error.WriteLine("usage: kitaqfc src2asm <source.c:line> [--disasm=<dis.s>] [--funcsizes=<.funcsizes.txt>] [--context=N] [--out=<file>|-]");
            Environment.Exit(1);
            return true;
        }
    ...
    static bool TryRunRomDiffCommand(string[] argsArray)
    {
        ...
        {
            Console.Error.WriteLine("error KQ0000: romdiff requires <old.nes> <new.nes>");
            Console.Error.WriteLine("usage: kitaqfc romdiff <old.nes> <new.nes> [--old-func=<.funcsizes.txt>] [--new-func=<.funcsizes.txt>] [--old-map=<.map>] [--new-map=<.map>] [--out=<file>|-]");
            Environment.Exit(1);
            return true;
        }
    ...
    static void EmitTraceShortSummary(List<TraceStageStat> stats, long totalMs, List<string> sourceFilenames, string outputFilename)
    {
        try
        {
            ...
            Console.WriteLine("[trace] total=" + totalMs + "ms, " + compact);
    ...
    static void TryWriteFailureReproPackage(int exitCode)
    {
        ...
        try
        {
            ...
            if (string.IsNullOrWhiteSpace(root))
                root = Path.Combine(Environment.CurrentDirectory, "kitaqfc_fail_repro_" + DateTime.UtcNow.ToString("yyyyMMdd_HHmmssfff"));
            ...
            IoUtil.WriteAllTextUtf8Robust(Path.Combine(root, "command.txt"),
                "kitaqfc " + string.Join(" ", args.Select(QuoteArg)),
    ...
    static string GetKqDescription(int code)
    {
        switch ((ErrorCode)code)
        {
            ...
            case ErrorCode.LargeStructCopy: return "大きなstruct/unionコピーが発生しています。";
    ...
    // Parse decimal integers or hexadecimal values prefixed with 0x.
