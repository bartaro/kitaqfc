...
class Lowerer
{
    ...
    sealed class GlobalEnv
    {
        ...
        public void Build(Expr program)
        {
            ...
            AddIntrinsicReturn("__smul16x8", CType.UInt16);
            AddIntrinsicReturn("__mac16", CType.UInt16);
            AddIntrinsicReturn("__smac16", CType.UInt16);
            AddIntrinsicReturn("__sdot3_q1_7", CType.UInt16);
            AddIntrinsicReturn("__bit_test", CType.UInt8);
            AddIntrinsicReturn("__ppu_read_status", CType.UInt8);
            AddIntrinsicReturn("__nmi_ready", CType.UInt8);
            AddIntrinsicReturn("__pad_read1", CType.UInt8);
            AddIntrinsicReturn("__pad_read2", CType.UInt8);
            AddIntrinsicReturn("__pad_read1_safe", CType.UInt8);
            AddIntrinsicReturn("__pad_read2_safe", CType.UInt8);
            AddIntrinsicReturn("__pad_buttons", CType.UInt8);
            AddIntrinsicReturn("__pad_dirs", CType.UInt8);
            AddIntrinsicReturn("__mapper_id", CType.UInt8);
            AddIntrinsicReturn("__fds_available", CType.UInt8);
            AddIntrinsicReturn("__fds_disk_ready", CType.UInt8);
            AddIntrinsicReturn("__fds_side", CType.UInt8);
            AddIntrinsicReturn("__fds_error", CType.UInt8);
            AddIntrinsicReturn("__fds_load_file", CType.UInt8);
            AddIntrinsicReturn("__fds_save_file", CType.UInt8);
            AddIntrinsicReturn("__fds_load_overlay", CType.UInt8);
            AddIntrinsicReturn("__fds_load_bank", CType.UInt8);
            AddIntrinsicReturn("__fds_require_bank", CType.UInt8);
            AddIntrinsicReturn("__fds_current_bank", CType.UInt8);
            AddIntrinsicReturn("__fds_is_bank_resident", CType.UInt8);
            AddIntrinsicReturn("__fds_overlay_function_count", CType.UInt8);
            AddIntrinsicReturn("__fds_overlay_farcall", CType.UInt8);
            AddIntrinsicReturn("__fds_farcall", CType.UInt8);
            AddIntrinsicReturn("__fds_file_exists", CType.UInt8);
            AddIntrinsicReturn("__fds_file_size", CType.UInt16);
        }
    ...
    static Expr ApplyIndexRangeAndLints(FunctionCtx ctx, Expr sourceExpr, Expr arrayExpr, Expr indexExpr)
    {
        ...
        if (TryInferIntRange(ctx, idx, out int imin, out int imax))
        {
            ...
            if (TryGetArrayLength(ctx, bt, out len) && len > 0)
            {
                ...
                // For a proven in-range byte-sized array index, insert a u8 cast without adding a safe-index type annotation.
                else if (len <= 0xFF && imin >= 0 && imax <= 0xFF && !HasTopLevelCast(idx))
                {
                    idx = Expr.Make(Tag.Cast, CType.UInt8, idx).WithSource(idx.Source);
                }
            }
        }
...
// ---------- Function lowering ----------
    ...
    static List<Expr> LowerStatement(FunctionCtx ctx, Expr stmt)
    {
        ...
        if (stmt.Match(Tag.For, out init, out test, out induct, out body))
        {
            ...
            // In this FC path, continue rewriting occurs only when condition lowering extracted a prefix; ordinary induct-only loops are unchanged.
            // and before subsequent tests (in induct). Also ensure 'continue' executes induct+prefix.
            if (testPrefix.Count > 0)
            {
                ...
                // Important: the backend's FOR uses the loop-top label as ContinueLabel, so 'continue'
                // would otherwise skip the induct. We rewrite continues to run induct+prefix first.
...
if (stmt.MatchAny(Tag.If, out parts))
    ...
    // Prefix matching continue nodes through sequences, ifs and recognized integer-valued switch cases.
    // Nested For nodes are left intact; other wrappers and loop forms are not traversed by this helper.
...
static Expr LowerIfChain(FunctionCtx ctx, Expr[] parts, FilePosition src)
    ...
    static Expr LowerExpr(FunctionCtx ctx, Expr expr, bool allowExtract, List<Expr> prefix)
    {
        ...
        if (expr.Match(Tag.Assign, out lval, out rval))
        {
            ...
            // - assigning through an explicit top-level cast written by the caller
            // Remember the original RHS cast before temporary extraction can hide it from assignment lints.
            bool safeIndexExplicitCast = HasTopLevelCast(rval);
            if (IsSafeIndex(lt) && !IsSafeIndex(rt))
            {
                int __v;
                if (!TryConstInt(newR, out __v) && !safeIndexExplicitCast)
                {
                    Program.Warning(expr.Source, ErrorCode.SafeIndexMix,
                     ...
                     !safeIndexExplicitCast && !IsGeneratedTempLvalue(newL))
            {
                Program.Warning(expr.Source, ErrorCode.SafeIndexMix,
...
// Lint: __range(min,max) variable assigned an out-of-range constant.
        ...
        if (expr.Match(Tag.ShiftLeft, out left, out right) || expr.Match(Tag.ShiftRight, out left, out right))
        {
            ...
            if (allowExtract)
            {
                ...
                // tmpVal = left
                // Retain signedness so each generated right shift propagates the original sign bit.
	        ...
	        if (expr.Match(Tag.BitwiseNot, out sub) || expr.Match(Tag.LogicalNot, out sub))
        {
            ...
            {
                // This FC comparison prepass spills non-atoms before the later constant-preserving comparison helper.
            ...
            // For comparisons, spill complex 8/16-bit operands into temporaries. The
            // NES compare path stays much more predictable when both sides are atoms.
            if (allowExtract && IsComparisonTag(binTag))
            {
    ...
    // Recognize only generated temporary names used to suppress safe-index assignment noise.
    static bool IsGeneratedTempLvalue(Expr expr)
    {
        string name;
        return expr != null &&
               expr.Match(Tag.Name, out name) &&
               !string.IsNullOrEmpty(name) &&
               (name.StartsWith("__t8_", StringComparison.Ordinal) ||
                name.StartsWith("__t16_", StringComparison.Ordinal));
    }
    ...
    static void CollectNameUses(Expr expr, HashSet<string> used)
    {
        ...
        for (int i = 1; i < args.Length; i++)
        {
            ...
            // Also count symbolic bases embedded in raw assembly operands.
            AsmOperand operand = a as AsmOperand;
            if (operand != null && operand.Base.HasValue && !string.IsNullOrEmpty(operand.Base.Value))
                used.Add(operand.Base.Value);
        }
    }
